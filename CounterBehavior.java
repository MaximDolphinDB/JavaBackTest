...
public class CounterBehavior extends TradeBehavior {
    ...
    public static void afterStockBar(){
        /*
        * 1. 计算仓位的动态盈亏(由于所有Position在创建的时候pre_price == price, 所以第一个K线的动态盈亏始终为0.0)
        * 2. 更新股票仓位的pre_price为最新price
        * */
        ...
        for (String symbol: stockPosition.keySet()){
            if (stock_k_dict.containsKey(symbol)){
                ...
                for (int i=1; i<k; i++){
                    ...
                    double realTimeProfit = (close - position.pre_price) * position.vol;

                    // 更新当前实时盈亏
                    config.realTimeProfit += realTimeProfit;
                    config.stockRealTimeProfit += realTimeProfit;
                    positionList.get(i).profit = realTimeProfit;

                    // 更新当前标的pre_price为price
                    positionList.get(i).pre_price = close;
                }
    ...
    public static void afterFutureBar(){
        /*
         * 1. 计算仓位的动态盈亏(由于所有Position在创建的时候pre_price == price, 所以第一个K线的动态盈亏始终为0.0)
         * 2. 更新期货多头/空头仓位的pre_price为最新price
         * */
        Integer LS;
        ...
        for (String symbol: futureLongPosition.keySet()){
            if (stock_k_dict.containsKey(symbol)){
                ...
                for (int i=1; i<k; i++){
                    ...
                    double realTimeProfit = (close - position.pre_price) * position.vol * LS;

                    // 更新当前实时盈亏
                    config.realTimeProfit += realTimeProfit;
                    config.futureRealTimeProfit += realTimeProfit;
                    positionList.get(i).profit = realTimeProfit;
                    positionList.get(i).margin += realTimeProfit;
    ...
    public static void executeStock(String symbol, Double price, Double vol,
        ...
        */
        ...
        // 初始化Summary视图对象
        StockPosition pos = new StockPosition(price, vol, min_timestamp, max_timestamp, 0);
    ...
    public static void executeFuture(String order_type, String symbol, Double price, Double vol,
        ...
        */
        ...
        FuturePosition pos = new FuturePosition(price, vol, margin_rate, pre_settle, min_timestamp, max_timestamp, 0); // 这里已经预先添加了hold_days=0, 相当于无论什么情况下, 只要下单, 那么一开始就没有走完一个day, hold_days=0
        ...
        if (order_type.equals("long")){
            ...
            if (!config.getFutureLongSummary().containsKey(symbol)){
                FutureSummary summary = new FutureSummary(price, vol, static_profit, static_loss, dynamic_profit, dynamic_loss, price, price); // 这里history_min & history_max 都是当前价格
                config.futureLongSummary.put(symbol, summary); // 加入多头的持仓视图中
            }else{
                // 需要先更新止盈止损属性
                FutureSummary summary = config.futureLongSummary.get(symbol);
                FutureSummary summary_updated = summary.update(summary, price, vol, static_profit, static_loss, dynamic_profit, dynamic_loss);
                config.futureLongSummary.put(symbol, summary_updated);
            }
        }else{
            // 将该仓位加入对应的持仓中
            if (!config.getFutureShortSummary().containsKey(symbol)){
                ...
                // 需要先更新止盈止损属性
                FutureSummary summary = config.futureShortSummary.get(symbol);
                FutureSummary summary_updated = summary.update(summary, price, vol, static_profit, static_loss, dynamic_profit, dynamic_loss);
                config.futureShortSummary.put(symbol, summary_updated);
            }
        }
...
//        LinkedHashMap<String, ArrayList<FuturePosition>> futurePos;
    ...
    public static void monitorStockPosition(boolean order_sequence){
        ...
        */
        ...
        // 获取当前配置实例
        BackTestConfig config = BackTestConfig.getInstance();
        LinkedHashMap<String, ArrayList<StockPosition>> pos = config.getStockPosition();
        if (pos.isEmpty()){
            return ;  // 当前没有持仓
        }
        ...
        for (String symbol: pos.keySet()){
            ...
            ArrayList<StockPosition> positionList = pos.get(symbol);
            int i = 0;
